...
class account {
    ...
    public function loginAccount($username,$password){
        $db = new connect();
        $strQuery = "select * from account where user_name='$username' AND pass_word = '$password' AND role = 1";
        $r = $db->getInstance($strQuery);
        return $r;
    }
    public function addAccount($name,$username,$password,$email,$phone,$role){
        $db = new connect();
        $strQuery = "INSERT INTO account(id,name,user_name,pass_word,email,sdt,role) VALUES(NULL,'$name','$username',MD5('$password'),'$email','$phone','$role')";
        $r = $db->exec($strQuery);
        return $r;
    }
